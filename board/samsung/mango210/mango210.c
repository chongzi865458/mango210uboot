...
int board_late_init (void)
{
	...
	case BOOT_NAND:
		...
		if(default_boot_mode) {		// using default environment
			sprintf(boot_cmd, "nand read %08x 600000 500000;nand read %08x B00000 300000; bootm %08x %08x"
			...
			sprintf(boot_cmd, "root=ramfs devfs=mount console=ttySAC2,115200 ubi.mtd=4 ubi.mtd=5");
	...
	case BOOT_MMCSD:
		...
		if((~Inp32(0xe0200c04)) & 0x6) { // Linux Recovery Booting Mode
			...
			sprintf(boot_cmd, "movi read kernel %08x; bootm %08x"
					...
					, MEMORY_BASE_ADDRESS + 0x8000);
			setenv("bootcmd", boot_cmd);
			sprintf(boot_cmd, "root=/dev/mmcblk0p2 rootfstype=ext3 init=/init console=ttySAC2,115200 rootdelay=10");	// Gnome Booting Mode
			setenv("bootargs", boot_cmd);
			...
			setenv("bootcmd", boot_cmd);
			sprintf(boot_cmd, "root=/dev/mmcblk0p3 rootfstype=ext3 console=ttySAC2,115200 rootdelay=10 recovery");
			setenv("bootargs", boot_cmd);
			sprintf(boot_cmd, "0");
		...
		if(default_boot_mode) {	// using default environment
			sprintf(boot_cmd, "movi read kernel %08x; bootm %08x"
					...
					, MEMORY_BASE_ADDRESS + 0x8000);
			setenv("bootcmd", boot_cmd);
			sprintf(boot_cmd, "root=/dev/mmcblk0p2 rootfstype=ext3 init=/init console=ttySAC2,115200 rootdelay=6");	// Android Booting Mode
			setenv("bootargs", boot_cmd);
		}
		break;
	}
